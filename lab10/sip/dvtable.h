...
typedef struct distancevector {
	int nodeID;		        //源节点ID
	dv_entry_t* dvEntry;	//一个包含N个dv_entry_t的数组, 
		//其中每个成员包含目标节点ID和从该源节点到该目标节点的代价. 
		//N是重叠网络中总的节点数.
} dv_t;
...
//再给你一个锅
//更新距离向量表
// 输入参数是：邻居代价表、距离向量表、距离向量表互斥量、路由更新报文
// 如果成功返回1
// 如果失败返回-1
int route_dvt_update(nbr_cost_entry_t *nct, dv_t *dvt, routingtable_t * rtt,
	 pkt_routeupdate_t *data, int fromNodeID, pthread_mutex_t *dv_mutex);
#endif
