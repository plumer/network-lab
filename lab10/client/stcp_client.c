...
//	h -> checksum = checksum(&segment);
	...
	p -> state = SYNSENT;
	do {
		int send_res = sip_sendseg(stcp_client_conn, nodeID, &segment);
		assert(send_res == 1);

		usleep(SYN_TIMEOUT/500);
...
int stcp_client_disconnect(int sockfd) {
	...
	p -> state = FINWAIT;
	do {
		int send_res = sip_sendseg(stcp_client_conn, p -> server_nodeID, &fin_seg);
		if (send_res != 1) {
			print_pos();
...
void *seghandler(void* arg) {
	...
	while (1) {
		...
		int src_node = -1;
		int r = sip_recvseg(stcp_client_conn, &src_node, &seg);
		if (r == -1) {
		...
		client_tcb_t * tcbp = NULL;
		// search for the right client tcb.
		int i = 0;
		for (; i < MAX_TRANSPORT_CONNECTIONS; ++i) if (tcb_table[i]) {
			if (tcb_table[i] -> client_portNum == h -> dest_port &&
				tcb_table[i] -> server_nodeID == src_node) {
				tcbp = tcb_table[i];
			}
		}
		if (tcbp == NULL) {
			printf("unknown segment to client port %d, src node = %d, type = %s",
				h -> dest_port, src_node, seg_type_str[h -> type]);
			continue;
		}

		printf("received a pack to port %d, src node = %d, type = %s\n",
			h -> dest_port, src_node, seg_type_str[h -> type]);
		if (h -> type == SYNACK) {
			// find the tcb entry with corresponding h -> dest_port
//			printf("received a SYNACK segment to port %d\n", h -> dest_port);
			if (tcbp -> state == SYNSENT) {
				tcbp -> state = CONNECTED;
				tcbp ->  next_seqNum = h -> seq_num;
				printf("regular SYNACK\n");
				// raise(SIGUSR1);
				continue;
			} else if (tcbp -> state == CONNECTED) {
				printf("repeated SYNACK\n");
				continue;
			} else {
				printf("panic: tcb (port %d) is not SYNSENT\n",
						tcbp -> client_portNum);
				assert(0);
			}
			...
			if (tcbp -> state == FINWAIT) {
				tcbp -> state = CLOSED;
				tcbp -> next_seqNum = h -> seq_num;
				printf("regular FINACK\n");
				// raise(SIGUSR1);
				continue;
			} else if (tcbp -> state == CLOSED) {
				printf("repeated FINACK\n");
				continue;
			} else {
				printf("panic: tcb (port %d) is not FINWAIT\n",
						tcbp -> client_portNum);
				assert(0);
			}
		...
		if (tcbp -> sendBufHead == NULL) {	// nothing in buf
			...
			if (timelag >= SENDBUF_POLLING_INTERVAL / 1000000) {
				while (p != tcbp -> sendBufunSent) {
					sip_sendseg(stcp_client_conn, tcbp -> server_nodeID, &(p -> seg) );
			...
			while (tcbp -> unAck_segNum < GBN_WINDOW) {
				if (tcbp -> sendBufunSent) {
					sip_sendseg(stcp_client_conn, tcbp -> server_nodeID, &(tcbp -> sendBufunSent -> seg) );
			...
			assert(timelag >= 0 && timelag <= SENDBUF_POLLING_INTERVAL/1000000);
			pthread_mutex_unlock(tcbp -> buf_mutex);
			usleep(SENDBUF_POLLING_INTERVAL/1000 - timelag * 1000);
		}	
	}
