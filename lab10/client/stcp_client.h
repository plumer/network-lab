...
typedef struct client_tcb {
	...
	pthread_mutex_t * buf_mutex;      //发送缓冲区互斥量
...
// 它还针对TCP套接字描述符conn初始化一个STCP层的全局变量, 
// 该变量作为sip_sendseg和sip_recvseg的输入参数.
// 最后, 这个函数启动seghandler线程来处理进入的STCP段. 
// 客户端只有一个seghandler.
//
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//
...
// 该TCB中的所有字段都被初始化. 例如, TCB state被设置为CLOSED，
//     客户端端口被设置为函数调用参数client_port. 
...
// 这个函数用于连接服务器. 它以套接字ID, 服务器节点ID和服务器的端口
//     作为输入参数. 套接字ID用于找到TCB条目.  
// 这个函数设置TCB的服务器节点ID和服务器端口号,  
// 然后使用sip_sendseg()发送一个SYN段给服务器.  
// 在发送了SYN段之后, 一个定时器被启动. 
...
// 因为用户数据被分片为固定大小的STCP段, 
// 所以一次stcp_client_send调用可能会产生多个segBuf
// 被添加到发送缓冲区链表中. 
// 如果调用成功, 数据就被放入TCB发送缓冲区链表中, 根据滑动窗口的情况,
...
// 这个函数用于断开到服务器的连接. 它以套接字ID作为输入参数. 
// 套接字ID用于找到TCB表中的条目.  
// 这个函数发送FIN段给服务器. 
// 在发送FIN之后, state将转换到FINWAIT, 并启动一个定时器.
// 如果在最终超时之前state转换到CLOSED, 则表明FINACK已被成功接收. 
// 否则, 如果在经过FIN_MAX_RETRY次尝试之后,
...
// seghandler被设计为一个调用sip_recvseg()的无穷循环. 
// 如果sip_recvseg()失败, 则说明到SIP进程的连接已关闭,
