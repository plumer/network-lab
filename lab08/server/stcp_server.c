...
void stcp_server_init(int conn) {
	...
	signal(SIGINT, stcp_exceptionClose);
}
...
int stcp_server_sock(unsigned int server_port) {
	...
	// 建立新的socket，返回服务器套接字【ID:0-9】
		...
		tcb[cur]->expect_seqNum = 0;
		tcb[cur]->recvBuf = (char *)malloc(RECEIVE_BUF_SIZE*sizeof(char));
				...
				switch(tcb[k]->state){
					...
					case CONNECTED:
						     if (tmp.header.type == FIN){
							     ...
							     if (sip_sendseg(sip_connfd, &ack)) printf("\tGOOD MSG: send DATAACK ok\n");
