...
#include <assert.h>
const char * seg_str[] = {
	"SYN", "SYNACK", "FIN", "FINACK", "DATA", "DATAACK"
};
			...
			case SEGSTOP2:
				if (buf[0] == '#') {
					...
					if (seglost(segPtr) == 1) return 1;
						...
						if (checkchecksum(segPtr) == 1) return 0;
						else {
...
//	return 0;
	...
	if(random<PKT_LOSS_RATE*100) {
		print_pos();
		printf("lost package: type = %s, seq_num = %d, ack_num = %d",
				seg_str[segPtr->header.type], segPtr -> header.seq_num,segPtr -> header.ack_num);
		//50%可能性丢失段
		if(rand()%2==0) {
			printf("seg lost!!!\n");
			return 1;
